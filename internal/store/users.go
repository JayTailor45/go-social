...
func (s *UserStore) CreateAndInvite(ctx context.Context, user *User, token string, expiry time.Duration) error {
	return withTx(s.db, ctx, func(tx *sql.Tx) error {
		...
		// create user invite
		if err := s.createUserInvite(ctx, tx, token, expiry, user.ID); err != nil {
			return err
		}
		return nil
	})
...
func (s *UserStore) createUserInvite(ctx context.Context, tx *sql.Tx, token string, expiry time.Duration, userID int64) error {
...
func (s *UserStore) Activate(ctx context.Context, token string) error {
	return withTx(s.db, ctx, func(tx *sql.Tx) error {
		// steps:
		// find user which the token belongs to
		// update the user
